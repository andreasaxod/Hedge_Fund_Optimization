...
sheet_name       = "UCITS_USD"
min_weight       = 0.05
max_weight       = 0.25
min_funds        = 4
max_funds        = 20
TARGET_SUBSETS   = 30000   # adjust for speed/coverage
periods_per_year = 12

# Risk-free (3M Libor)
LIBOR_3M_ANNUAL = 0.0435
rf_annual       = LIBOR_3M_ANNUAL
rf_monthly      = (1 + rf_annual)**(1/periods_per_year) - 1

# Strategy-level allocation bounds
strategy_bounds = {
    'Relative Value':    (0.15, 0.19),
    'Long Short Equity': (0.45, 0.55),
    'Event Driven':      (0.11, 0.15),
    'Global Macro':      (0.08, 0.12),
    'Multi-Strategy':    (0.05, 0.07),
    'CTA':               (0.05, 0.05),
}

print("🔒 Random seed set to", RANDOM_SEED)
print("🇪🇺 UCITS USD PORTFOLIO OPTIMIZATION")
...
def find_stress_periods(
    ...
    # Expand and collapse to (start, end)
...
# LOAD PORTFOLIO RETURNS & PARSE DATES
...
# Normalize strategy labels to match keys in strategy_bounds
def _norm_strat(s):
    ...
    if 'cta' in sl or 'managed futures' in sl: return 'CTA'
    return s0
...
def calculate_fund_requirements():
    ...
    for strat,(lb,ub) in strategy_bounds.items():
        ...
        if avail_cnt == 0:
            if lb > 0:
                raise RuntimeError(
                    f"Infeasible: '{strat}' lower bound {lb:.2f} but 0 funds available."
                )
            req[strat] = (0, 0)
            continue
...
def generate_ucits_subsets(target_subsets=5000, seed=None):
    if seed is not None:
        random.seed(seed); np.random.seed(seed)
    req = calculate_fund_requirements()
    combos = []
    for rv in range(req['Relative Value'][0], req['Relative Value'][1]+1):
      for lse in range(req['Long Short Equity'][0], req['Long Short Equity'][1]+1):
        for ed in range(req['Event Driven'][0], req['Event Driven'][1]+1):
          for gm in range(req['Global Macro'][0], req['Global Macro'][1]+1):
            for ms in range(req['Multi-Strategy'][0], req['Multi-Strategy'][1]+1):
              for cta in range(req['CTA'][0], req['CTA'][1]+1):
                total = rv+lse+ed+gm+ms+cta
                if min_funds <= total <= max_funds:
                    combos.append((rv,lse,ed,gm,ms,cta))
    if not combos:
        return []
    samples_per = max(1, target_subsets // max(1, len(combos)))
    subsets = []
    strat_keys = list(strategy_bounds.keys())
    seen = set()  # avoid duplicates
    for idx, combo in enumerate(combos):
        for sample_idx in range(samples_per):
            random.seed((seed or 0) * 1000003 + idx * 97 + sample_idx)
            sub = []
            infeasible = False
...
def project_sharpe_fast(sub):
    ...
    # warm start toward mean-variance tangency direction
    try:
...
subsets = generate_ucits_subsets(TARGET_SUBSETS, seed=RANDOM_SEED)
print(f"✅ Generated {len(subsets)} subsets")

results=[]; start_time=time.time(); last=start_time
...
custom_bench_date_col   = "Date"
custom_bench_return_col = "HFRU Hedge Fund Composite USD Index"
...
def load_custom_benchmark(path, sheet, date_col, return_col, start, end, Rdf_index):
    ...
    # Detect percent units and convert to decimal if needed (slightly safer guard)
    med = s.dropna().abs().median()
    if np.isfinite(med) and med > 1:
        s = s / 100.0
    elif np.isfinite(med) and 0.5 < med <= 1.0:
        s = s / 100.0
...
def report_ucits(best, name, bench_series):
    ...
    print(f"\n🇪🇺 UCITS {name} ({len(sub)} funds) vs {best_bench_name}:")
...
def plot_cum_and_corr(best, name, bench_series, label_bench, periods_per_year=12,
                      # Auto-calibrated benchmark stress
                      show_stress=True,
                      stress_kwargs=None,
                      # Relative stress (portfolio underperformance)
                      show_relative_stress=True,
                      relative_kwargs=None,
    ...
    if y.dropna().empty or x.dropna().empty:
        print(f"⚠️ Skipping plots for {name}: no overlapping non-NaN data with benchmark.")
        return
    y = y.dropna(); x = x.dropna()
    idx = y.index.intersection(x.index)
    y = y.loc[idx]; x = x.loc[idx]

    # Cumulative growth & rolling correlation
...
if best_so is not None:
    report_ucits(best_so, "Max-Sortino", chosen_bench)
...
report_ucits(best_sh, "Max-Sharpe", chosen_bench)
...
if results:
    ...
    if len(sharpe_df) > 0:
        ...
        plt.suptitle('UCITS USD - SHARPE OPTIMIZED PORTFOLIOS', fontsize=16, fontweight='bold')
        plt.subplot(2, 3, 1)
    ...
    if len(sortino_df) > 0:
        plt.figure(figsize=(15, 10))
        plt.suptitle('UCITS USD - SORTINO OPTIMIZED PORTFOLIOS', fontsize=16, fontweight='bold')
        plt.subplot(2, 3, 1)
...
def plot_k_comparison_vs_benchmark(results, Rdf, bench_series, ks=(6,8,10,12,14,16,18,20),
    ...
    plt.title('UCITS USD - Best Portfolio vs Benchmark by Fund Count (k)')
    ...
    for k, best_k in sorted(candidates.items()):
        ...
        if len(y) < 12:
            print(f" Skipping k={k}: insufficient overlap with benchmark.")
            continue
...
if results:
    ...
    # 2) Best-per-k frontier (separately for Sharpe and Sortino)
    ...
    #  Pooled frontier (max across Sharpe/Sortino for each k)
